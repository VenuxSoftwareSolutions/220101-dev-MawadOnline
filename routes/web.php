 ...
 */
 ...
 Route::get('/test/download', function () 
 {
    ...
    $folderId = '11uXjyx5qeLc96GaVDLHneDNI_EpEUe6M'; // Replace with the public folder ID
    $saveDirectory = 'downloads'; // Directory to save the files
    downloadPublicGoogleDriveFolder($folderId, $saveDirectory);
...
function downloadPublicGoogleDriveFolder($folderId, $saveDirectory) {
    // Ensure the save directory exists
    if (!is_dir($saveDirectory)) {
        mkdir($saveDirectory, 0777, true);
    }

    // Fetch the folder's content (Google Drive renders HTML)
    $folderUrl = "https://drive.google.com/drive/folders/{$folderId}";
    $htmlContent = file_get_contents($folderUrl);

    // Extract file links using a regular expression
    preg_match_all('/"([a-zA-Z0-9_-]{33})"/', $htmlContent, $matches);

    // Loop through all found file IDs and download them
    foreach (array_unique($matches[1]) as $fileId) {
        // Generate a direct download link for each file
...
function getFileNameFromId($fileId) {
    // You can fetch the file name by requesting the file metadata from Google Drive
    $metadataUrl = "https://www.googleapis.com/drive/v3/files/{$fileId}?fields=name&key=YOUR_API_KEY"; 
    $metadata = file_get_contents($metadataUrl);
    $metadataArray = json_decode($metadata, true);
    
    return isset($metadataArray['name']) ? $metadataArray['name'] : "file_{$fileId}.jpg";
}
