...
class ProductService
{
    public function store(array $data)
    {
        ...
        foreach($collection['from_shipping'] as $key => $from_shipping){
            if(($from_shipping != null) && ($collection['to_shipping'][$key]!= null)&& ($collection['shipper'][$key]!= null)&& ($collection['estimated_order'][$key]!= null)&& ($collection['estimated_shipping'][$key]!= null)&& ($collection['paid'][$key]!= null)){
                $current_data = [];
                ...
                foreach ($variants_data as $id => $variant){
                    ...
                    if(array_key_exists('shipping_details', $variant)){
                        foreach($variant['shipping_details']['from'] as $key => $from){
                            if(($from != null) && ($variant['shipping_details']['to'][$key]!= null)&& ($variant['shipping_details']['shipper'][$key]!= null)&& ($variant['shipping_details']['estimated_order'][$key]!= null)&& ($variant['shipping_details']['estimated_shipping'][$key]!= null)&& ($variant['shipping_details']['paid'][$key]!= null)){
                                $current_shipping = [];
    ...
    public function update(array $data, Product $product_update)
    {
        ...
        if (array_key_exists('variant', $data)) {
            foreach($collection['variant']['sku'] as $key => $sku){
                ...
                if(array_key_exists('sample_pricing', $data['variant'])){
                    ...
                    $variants_data[$key]['sample_pricing'] = 0;
                    $variants_data[$key]['sample_description'] = $data['sample_description'];
                    $variants_data[$key]['sample_pricing'] = $data['sample_price'];
                }
        ...
        if(!isset($data['activate_attributes'])){
            ...
            $deleted_attributes_general = ProductAttributeValues::where('id_products', $id)->where('is_general', 1)->whereNotIn('id_attribute', $new_ids_attributes_general)->delete();
    ...
    public function draft(array $data, Product $product_draft){
        ...
        if(!isset($data['activate_attributes'])){
            ...
            if(count($pricing) > 0){
                $all_data_to_insert = [];
                $all_data_to_insert_product_draft = [];
                ...
                foreach($pricing['from'] as $key => $from){
                    if($from != null){
                        ...
                        array_push($all_data_to_insert_product_draft, $current_data);
                    }
                    
                }

                PricingConfiguration::where('id_products', $product_draft->id)->delete();
                PricingConfiguration::insert($all_data_to_insert_product_draft);
            }
            unset($collection['is_parent']);
            ...
            if(count($variants_data) > 0){
                foreach ($variants_data as $id => $variant){
                    ...
                    if($product != null){
                        ...
                        $new_ids_attributes = array_keys($variant['attributes']);
                        $deleted_attributes = ProductAttributeValues::where('id_products', $id)->whereNotIn('id_attribute', $new_ids_attributes)->delete();
            ...
            $new_ids_attributes_general = array_keys($general_attributes_data);
            $deleted_attributes_general = ProductAttributeValues::where('id_products', $product_draft->id)->whereNotIn('id_attribute', $new_ids_attributes_general)->delete();
