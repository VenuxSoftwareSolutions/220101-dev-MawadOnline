@foreach ($attributes as $attribute)
    @if (in_array($attribute->id, $general_attributes_ids_attributes))
        <div class="col-md-4 mb-3 attribute-variant-{{ $attribute->id }}">
        ...
        @switch ($attribute->type_value)
            ...
            @case ('list')
                ...
                @endphp
                <div class="col-md-8 mb-3 attribute-variant-{{ $attribute->id }}">
                    <select class="form-control" @if(isset($general_attributes[$attribute->id]->old_value)) style="border-color: #FF3C50 !important;" data-toggle="tooltip" data-html="true" title="Modified and old video provider is: {{ isset($general_attributes[$attribute->id]->old_value) }}" @endif name="attribute_generale-{{ $attribute->id }}">
            ...
            @case ('boolean')
                <div class="col-md-8 mb-3 attribute-variant-{{ $attribute->id }}" style="padding-top: 10px">
                    <label style="margin-right: 15px">
                        <input type="radio" class="attributes" name="attribute_generale-{{ $attribute->id }}" @if(isset($general_attributes[$attribute->id]->old_value))  data-toggle="tooltip" data-html="true" title="Modified and old value is: {{ $general_attributes[$attribute->id]->old_value }}" style=" accent-color:red !important;" @endif @if($general_attributes[$attribute->id]->value == "yes") checked @endif name="boolean" value="yes">Yes
                    </label>
                    <label>
                        <input type="radio" class="attributes" name="attribute_generale-{{ $attribute->id }}" name="boolean" @if(isset($general_attributes[$attribute->id]->old_value))  data-toggle="tooltip" data-html="true" title="Modified and old value is: {{ $general_attributes[$attribute->id]->old_value }}" style=" accent-color:red !important;" @endif @if($general_attributes[$attribute->id]->value == "no") checked @endif value="no"> No
                    </label>
                </div>
                @break;
        @endswitch
