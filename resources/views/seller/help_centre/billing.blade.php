@extends('seller.layouts.app')
@php
    use Carbon\Carbon;
@endphp

<style>
    /* Your existing styles */
    @media (max-width: 540px) {
        ul.nav.nav-tabs.shop {
            background: #f8f9fa;
            margin: 0;
            display: block !important;
            justify-content: space-between;
            align-items: center;
        }
    }

    @media (min-width: 822px) and (max-width: 1198px) {
        ul.nav.nav-tabs.shop {
            background: #f8f9fa;
            margin: 0;
            display: block !important;
            justify-content: space-between;
            align-items: center;
        }
    }

    @media (min-width: 579px) and (max-width: 805px) {
        ul.nav.nav-tabs.shop {
            background: #f8f9fa;
            margin: 0;
            display: block !important;
            justify-content: space-between;
            align-items: center;
        }
    }

    .color-modified {
        border: 2px dashed #e8c068e8 !important;
        box-shadow: 0 0 5px rgba(255, 204, 0, 0.5) !important;
        transition: border-color 0.3s ease, box-shadow 0.3s ease !important;
    }

    .align-items-center {
        -ms-flex-align: center !important;
    }

    .row.card-adjust {
        display: -ms-flexbox;
        display: flex;
        -ms-flex-wrap: wrap;
        flex-wrap: wrap;
        margin-right: -15px;
        margin-left: -15px;
        flex-direction: column;
        align-content: stretch;
        justify-content: space-evenly;
        align-items: flex-start;
    }

    .color-modified-file {
        color: #e8c068e8
    }

    .highlighted-tab {
        border-color: red !important;
    }

    .btn-primary {
        background-color: #a2b8c6 !important;
        border-color: #a2b8c6 !important;
    }

    .swal2-confirm {
        background-color: var(--success) !important;
        border-color: none !important;
    }

    #social-icon-two {
        display: flex !important;
        justify-content: center !important;
        align-items: center !important;
    }

    .swal2-confirm:hover {
        border-color: none !important;
    }

    .orange-text {
        color: #b9c9d4;
    }

    .Grand-title {
        padding-left: 0px !important;
    }

    .custom-file-input:lang(en)~.custom-file-label::after {
        content: "Browse";
        background-color: #a2b8c6;
        color: #fff;
    }

    .nav-link.active {
        background-color: #a2b8c6 !important;
        width: 200px;
    }

    .nav-tabs .nav-link {
        padding: 18px !important;
        text-align: start;
        border-top-left-radius: 1rem !important;
        border-top-right-radius: 1rem !important;
        border-bottom-left-radius: 1rem !important;
        border-bottom-right-radius: 1rem !important;
        width: 200px;
    }

    /* .nav-tabs .nav-link {
    background-color: white !important;
    } */

    .file-condition {
        font-size: 12px;
        font-weight: 450;
        margin-top: 3px;
        display: block;
    }

    li.nav-item {
        padding-bottom: 20px !important;
    }

    .optional {
        color: #b9c9d4;
        font-size: 14px;
    }

    span.aiz-side-nav-text:active {
        background-color: rgb(196, 196, 196);
        border-radius: 4% !important;
        width: auto;
        height: 50px;
    }

    .customer-color {
        background-color: #f77b0b !important;
        border: #f77b0b !important;
    }

    .card {
        border: 1px solid #dee2e6;
        border-radius: 0.25rem;
        margin-bottom: 10px;
        background-color: #f8f9fa;
    }

    .card-body {
        padding: 1rem;
    }

    .card-title {
        font-size: 1.25rem;
        margin-bottom: 0.75rem;
    }

    .card-deck {
        margin-right: 34px !important;
    }

    .card-link {
        color: #007bff;
        text-decoration: none;
    }

    .card-link:hover {
        text-decoration: underline;
    }

    .aiz-side-nav-item {
        display: flex;
    }

    .card {
        height: 94% !important;
    }

    .aiz-side-nav-list-content .aiz-side-nav-link {
        display: flex;
        flex-grow: 1;
        align-items: flex-start;
        padding: 10px 25px;
        font-size: 0.875rem;
        font-weight: 400;
        color: #252E4D;
    }

    .menu {
        margin-top: 42px;
    }

    ul.nav.nav-tabs.help-centre {
        display: -ms-flexbox;
        display: flex;
        -ms-flex-wrap: wrap;
        flex-wrap: nowrap;
        padding-left: 0;
        margin-bottom: 0;
        list-style: none;
        flex-direction: column;
        justify-content: flex-start;
        align-content: flex-start;
        align-items: baseline;
    }

    .container-flex {
        display: flex;
        margin-top: 24px;
        margin-left: 33px;
    }

    .nav-tabs-wrapper {
        flex: 0 0 250px;
        max-width: 250px;
    }

    .tab-content-wrapper {
        flex: 1;
        padding-left: 20px;
        margin-right: 37px;
    }

    .col-md-6.adjust-card {
        padding-right: 5px !important;
        padding-left: 5px !important;
    }

    .nav-tabs {
        border-bottom: none !important;
    }

    .nav-tabs .nav-item.show .nav-link,
    .nav-tabs .nav-link.active {
        color: #000 !important;
    }

    .content {
        padding-top: 20px;
    }

    .title-content {
        padding-bottom: 10px;
    }

    .title-business {
        display: inline-block;
        font-size: 1em;
        /* font-weight: bold; */
        font-size: 15px;


    }

    p.email-form-title1 {
        padding-bottom: 10px;
    }

    p.account-form-title1 {
        padding-bottom: 10px;
    }

    p.business-form-title1 {
        padding-bottom: 10px;
    }

    p.contact-form-title1 {
        padding-bottom: 10px;
    }

    p.Warehouse-form-title1 {
        padding-bottom: 10px;
    }

    p.Payout-form-title1 {
        padding-bottom: 10px;
        /* /font-weight: 500; */
    }

    span.package-title {
        font-size: 16px;
        font-weight: 500;
    }

    span.title-business {
        font-weight: 500;
    }

    span.registration-title {
        font-size: 16px !important;
        font-weight: 500 !important;
    }

    span.registration-content {
        font-size: 15px;

    }



    .tab-content>.active {
        display: block;
        text-align: justify;
    }

    .content-personal-info {
        padding-left: 20px;
    }


    .breadcrumb-nav {
        display: flex;
        align-items: center;
        padding-bottom: 25px;
    }

    .breadcrumb-nav a {
        text-decoration: none;
        color: #000;
    }

    .breadcrumb-nav .arrow-icon {
        margin: 0 10px;
    }

    .nav-link {
        background-color: #d3d3d3;
        width: 200px;
    }

    .text-nav {
        font-size: 14px;
    }
</style>
@section('panel_content')
    <div class="aiz-titlebar mt-4 mb-4">
        <div class="row align-items-center">
            <div class="col-md-12 mt-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('seller.help-center.index') }}">{{ __('help.Home') }}</a></li>
                        <span class="arrow-icon">
                            <img src="{{ asset('public/arrowIcon.png') }}" alt="arrow icon"
                                style="width: 17px; height: 17px;">
                        </span>
                        <li class="breadcrumb-item"><a href="/eshopProfile">{{ __('help.Billing') }}</a></li>
                        <span class="arrow-icon">
                            <img src="{{ asset('public/arrowIcon.png') }}" alt="arrow icon"
                                style="width: 17px; height: 17px;">
                        </span>
                        <li class="breadcrumb-item active" aria-current="page" id="current-page">
                            {{ __('help.Manage_eShop') }}</li>
                    </ol>

                </nav>
                <h1 class="h1"><b>{{ __('help.Billing') }}</b></h1>
            </div>
            <div class="container-flex">

                <div class="nav-tabs-wrapper">
                    <ul class="nav nav-tabs help-centre" id="myTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="eShop_Lease-tab" data-toggle="tab" href="#eShop_Lease"
                                role="tab" aria-controls="eShop_Lease" aria-selected="true"
                                data-title="{{ __('help.Manage_eShop') }}">
                                <b>{{ __('help.eShop_Lease') }}</b>
                                <p>{{ __('help.maintein-content') }}</p>
                            </a>
                        </li>


                        <li class="nav-item">
                            <a class="nav-link" id="personal-information-tab" data-toggle="tab" href="#personal-information"
                                role="tab" aria-controls="personal-information" aria-selected="false"
                                data-title="{{ __('help.Approval_Process_title') }}">
                                <b>{{ __('help.Approval_Process_title') }}</b>
                                <p>{{ __('help.Lease_card_content') }}</p>
                            </a>
                        </li>

                        <li class="nav-item">
                            <a class="nav-link" id="Sales_Transactions-tab" data-toggle="tab" href="#Sales_Transactions"
                                role="tab" aria-controls="Sales_Transactions" aria-selected="false"
                                data-title="{{ __('help.Sales_Transactions') }}">
                                <b>{{ __('help.Sales_Transactions') }}</b>
                                <p>{{ __('help.Lease_card_content') }}</p>
                            </a>
                        </li>

                        <li class="nav-item">
                            <a class="nav-link" id="Understanding-cont-tab" data-toggle="tab" href="#Understanding-cont"
                                role="tab" aria-controls="Understanding-cont" aria-selected="false"
                                data-title="{{ __('help.Understanding') }}">
                                <b>{{ __('help.Understanding') }}</b>
                                <p>{{ __('help.Lease_card_content') }}</p>
                            </a>
                        </li>

                        <li class="nav-item">
                            <a class="nav-link" id="Managing-tab" data-toggle="tab" href="#Managing" role="tab"
                                aria-controls="Managing" aria-selected="false" data-title="{{ __('help.Managing') }}">
                                <b>{{ __('help.Managing') }}</b>
                                <p>{{ __('help.Lease_card_content') }}</p>
                            </a>
                        </li>

                        <li class="nav-item">
                            <a class="nav-link" id="reviewingConst-tab" data-toggle="tab" href="#reviewingConst"
                                role="tab" aria-controls="reviewingConst" aria-selected="false"
                                data-title="{{ __('help.Reviewing') }}">
                                <b>{{ __('help.Reviewing') }}</b>
                                <p>{{ __('help.Lease_card_content') }}</p>
                            </a>
                        </li>


                    </ul>
                </div>
                <div class="tab-content-wrapper">
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="eShop_Lease" role="tabpanel"
                            aria-labelledby="eShop_Lease-tab">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-4">
                                        <div class="content">
                                            <h5 class="title-content"><b>{{ __('help.eShop_Lease') }}</b></h5>
                                            <span class="registration-title"><b>{{ __('help.Access-title') }}</b></span>
                                            <p class="registration-form-title">
                                                <span class="registration-content">{{ __('help.Access-content') }}</span>
                                            </p>

                                            <span class="registration-title"><b>{{ __('help.View_Lease') }}</b></span>
                                            <br>

                                            <span class="registration-title">{{ __('help.Current-title1') }}</span>
                                            <div class="content-personal-info">
                                                <p class="registration-form-title">

                                                    <span
                                                        class="registration-content">{{ __('help.Access-content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Access-content2') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Access-content3') }}</span>

                                                </p>
                                            </div>
                                            <span class="registration-title">{{ __('help.Current-title2') }}</span>
                                            <br>
                                            <table border="1" style="background-color: white; width: 100%;">
                                                <thead>
                                                    <tr>
                                                        <th>From Date</th>
                                                        <th>To Date</th>
                                                        <th>Charge for</th>
                                                        <th>Amount (AED)</th>
                                                        <th>Status</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td>26.Apr.24</td>
                                                        <td>25.May.24</td>
                                                        <td>eShop Lease</td>
                                                        <td>450.00</td>
                                                        <td>Unpaid</td>

                                                    </tr>
                                                    <tr>
                                                        <td>27.Apr.24</td>
                                                        <td>25.May.24</td>
                                                        <td>1 Sales Role</td>
                                                        <td>10.00</td>
                                                        <td>Unpaid</td>
                                                    </tr>
                                                    <tr>
                                                        <td>28.Apr.24</td>
                                                        <td>25.May.24</td>
                                                        <td>1 Inventory Management Role</td>
                                                        <td>10.00</td>
                                                        <td>Unpaid</td>
                                                    </tr>
                                                    <tr>
                                                        <td colspan="1" style="text-align: left;"><b>{{ __('help.Sub_Total') }} </b></td>
                                                        <td></td>

                                                        <td></td>
                                                        <td>470.00</td>
                                                        <td></td>
                                                    </tr>
                                                    <tr>
                                                        <td colspan="1" style="text-align: left;"><b>{{ __('help.VAT') }} </b></td>
                                                        <td></td>

                                                        <td></td>
                                                        <td>0.00</td>
                                                        <td></td>

                                                    </tr>
                                                    <tr>
                                                        <td colspan="1" style="text-align: left;"><b>{{ __('help.Total_Due:') }} </b></td>
                                                        <td></td>

                                                        <td></td>
                                                        <td>470.00</td>
                                                        <td></td>

                                                    </tr>
                                                </tbody>
                                            </table>
                                            <br>
                                            <span class="registration-title"><b>{{ __('help.Make') }}</b></span>
                                            <p class="registration-form-title">
                                                <span class="registration-content">{{ __('help.Make-content1') }}</span>
                                                <br>
                                                <span class="registration-content">{{ __('help.Make-content2') }}</span>
                                                <br>
                                                <span class="registration-content">{{ __('help.Make-content3') }}</span>
                                                <br>
                                                <span class="registration-content">{{ __('help.Make-content4') }}</span>
                                            </p>
                                            <br>
                                            <span class="registration-title"><b>{{ __('help.Download') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Download-content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Download-content2') }}</span>
                                            </p>


                                            {{-- start business section --}}
                                            <span
                                                class="registration-title"><b>{{ __('help.business_Info_title') }}</b></span>
                                            <br>
                                            <span class="registration-title">{{ __('help.Updating_Business') }}</span>
                                            <div class="content-personal-info">
                                                <p class="registration-form-title">
                                                    <span class="registration-title">{{ __('help.Fields') }}</span>
                                                    <br>
                                                    {{-- <span class="registration-content">{{ __('help.Access_the_eShop_content') }}</span> --}}

                                                    <span
                                                        class="registration-title">{{ __('help.English_Trade') }}</span><span
                                                        class="registration-content">{{ __('help.English_Trade_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Arabic_Trade') }}</span><span
                                                        class="registration-content">{{ __('help.Arabic_Trade_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Trade_License') }}</span><span
                                                        class="registration-content">{{ __('help.Trade_License_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Address_Details') }}</span><span
                                                        class="registration-content">{{ __('help.Address_Details_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.VAT_Registration') }}</span><span
                                                        class="registration-content">{{ __('help.VAT_Registration_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Civil_Defence') }}</span><span
                                                        class="registration-content">{{ __('help.Civil_Defence_content') }}</span>



                                                </p>
                                                <p class="registration-form-title">
                                                    <span class="registration-title">{{ __('help.Save_as_Draft') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Save_as_Draft_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Save_as_Draft_content2') }}</span>

                                                </p>
                                            </div>

                                            <span
                                                class="registration-title"><b>{{ __('help.Contact_Person_title') }}</b></span>
                                            <br>
                                            <span class="registration-title">{{ __('help.Updating_Contact') }}</span>
                                            <div class="content-personal-info">
                                                <p class="registration-form-title">
                                                    <span class="registration-title">{{ __('help.Fields') }}</span>
                                                    <br>
                                                    {{-- <span class="registration-content">{{ __('help.Access_the_eShop_content') }}</span> --}}

                                                    <span
                                                        class="registration-title">{{ __('help.First_Name_title') }}</span><span
                                                        class="registration-content">{{ __('help.First_Name_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Last_Name_title') }}</span><span
                                                        class="registration-content">{{ __('help.Last_Name_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Email_title') }}</span><span
                                                        class="registration-content">{{ __('help.Trade_License_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Address_Details') }}</span><span
                                                        class="registration-content">{{ __('help.Address_Details_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Mobile_title') }}</span><span
                                                        class="registration-content">{{ __('help.Mobile_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Nationality_title') }}</span>
                                                    <span
                                                        class="registration-content">{{ __('help.Nationality_content1') }}
                                                    </span>
                                                    <span
                                                        class="registration-title">{{ __('help.Date_title') }}</span><span
                                                        class="registration-content">{{ __('help.Date_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Emirate_title') }}</span><span
                                                        class="registration-content">{{ __('help.Emirate_content1') }}</span>

                                                </p>
                                                <p class="registration-form-title">
                                                    <span class="registration-title">{{ __('help.Save_as_Draft') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Save_as_Draft_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Save_as_Draft_content2') }}</span>

                                                </p>
                                            </div>

                                            {{-- start payout section --}}
                                            <span
                                                class="registration-title"><b>{{ __('help.payout_info_title') }}</b></span>
                                            <br>
                                            <span class="registration-title">{{ __('help.Updating_Payout') }}</span>
                                            <div class="content-personal-info">
                                                <p class="registration-form-title">
                                                    <span class="registration-title">{{ __('help.Fields') }}</span>
                                                    <br>
                                                    {{-- <span class="registration-content">{{ __('help.Access_the_eShop_content') }}</span> --}}

                                                    <span
                                                        class="registration-title">{{ __('help.Bank_Name') }}</span><span
                                                        class="registration-content">{{ __('help.Bank_Name_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Account_Name') }}</span><span
                                                        class="registration-content">{{ __('help.Account_Name_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Account_Number') }}</span><span
                                                        class="registration-content">{{ __('help.Account_Number_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Address_Details') }}</span><span
                                                        class="registration-content">{{ __('help.Address_Details_content') }}</span>
                                                    <br>
                                                    <span class="registration-title">{{ __('help.IBAN') }}</span><span
                                                        class="registration-content">{{ __('help.IBAN_content') }}</span>
                                                    <br>
                                                    <span class="registration-title">{{ __('help.Swift_Code') }}</span>
                                                    <span
                                                        class="registration-content">{{ __('help.Swift_Code_content') }}
                                                    </span>
                                                    <span
                                                        class="registration-title">{{ __('help.IBAN_Certificate') }}</span><span
                                                        class="registration-content">{{ __('help.IBAN_Certificate_content') }}</span>
                                                    <br>


                                                </p>
                                                <p class="registration-form-title">
                                                    <span
                                                        class="registration-title">{{ __('help.Save_as_Draft') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Save_as_Draft_content1') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.Save_as_Draft_content2') }}</span>

                                                </p>
                                            </div>

                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>

                        <div class="tab-pane fade" id="Understanding-cont" role="tabpanel"
                            aria-labelledby="Understanding-cont-tab">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-4">
                                        <div class="content">
                                            <h5 class="title-content"><b>{{ __('help.Lease') }}</b></h5>
                                            <span class="registration-title"><b>{{ __('help.Monthly') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Monthly_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Monthly_content2') }}</span>

                                            </p>
                                            <br>
                                            <span class="registration-title"><b>{{ __('help.Prorated') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Prorated_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Prorated_content2') }}</span>

                                            </p>

                                            <h5 class="title-content"><b>{{ __('help.Billing_title') }}</b></h5>
                                            <span
                                                class="registration-title"><b>{{ __('help.Billing_title1') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Billing_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Billing_content2') }}</span>

                                            </p>
                                            <br>
                                            <span class="registration-title"><b>{{ __('help.Payment_title') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Payment_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Payment_content2') }}</span>

                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="personal-information" role="tabpanel"
                            aria-labelledby="personal-information-tab">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-4">
                                        <div class="content">
                                            <h5 class="title-content"><b>{{ __('help.Account_creating') }}</b></h5>
                                            <span class="registration-title"><b>{{ __('help.Admin_Review ') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Admin_Review_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Admin_Review_content2') }}</span>
                                            </p>

                                            {{-- Status Notifications section --}}
                                            <span
                                                class="registration-title"><b>{{ __('help.Status_Notifications') }}</b></span>

                                            <br>
                                            {{-- <span class="registration-title">{{ __('help.Updating_Personal') }}</span> --}}
                                            <div class="content-personal-info">
                                                <p class="registration-form-title">

                                                    <span class="registration-title">{{ __('help.Pending') }}</span><span
                                                        class="registration-content">{{ __('help.Pending_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Pending_app') }}</span><span
                                                        class="registration-content">{{ __('help.Pending_app_content') }}</span>
                                                    <br>
                                                    <span class="registration-title">{{ __('help.under') }}</span><span
                                                        class="registration-content">{{ __('help.under_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Revision') }}</span><span
                                                        class="registration-content">{{ __('help.Revision') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Rejected') }}</span><span
                                                        class="registration-content">{{ __('help.rejected_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Approved') }}</span><span
                                                        class="registration-content">{{ __('help.Approved_content') }}</span>
                                                    <br>

                                                </p>
                                            </div>
                                            <span class="registration-title"><b>{{ __('help.Distinctive') }}</b></span>
                                            <br>

                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Distinctive_content') }}</span>

                                            </p>

                                            <span
                                                class="registration-title"><b>{{ __('help.Admin_Approval') }}</b></span>
                                            <br>
                                            <span class="registration-content">{{ __('help.approve_content') }}</span>

                                            <div class="content-personal-info">
                                                <p class="registration-form-title">

                                                    <span
                                                        class="registration-title">{{ __('help.Approve_Changes') }}</span><span
                                                        class="registration-content">{{ __('help.Approve_Changes_content') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-title">{{ __('help.Reject_Changes') }}</span><span
                                                        class="registration-content">{{ __('help.Reject_Changes_content') }}</span>

                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="tab-pane fade" id="Sales_Transactions" role="tabpanel"
                            aria-labelledby="Sales_Transactions-tab">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-4">
                                        <div class="content">
                                            <h5 class="title-content"><b>{{ __('help.Sales_Transactions') }}</b></h5>
                                            <span class="registration-title"><b>{{ __('help.Access_titles') }}</b></span>
                                            <p class="registration-form-title">
                                                <span class="registration-content">{{ __('help.Access_content') }}</span>

                                            </p>
                                            <span class="registration-title"><b>{{ __('help.View_Lease') }}</b></span>
                                            <br>



                                            <span
                                                class="registration-title">{{ __('help.View Sales Transactions') }}</span>
                                            <div class="content-personal-info">
                                                <p class="registration-form-title">

                                                    <span class="registration-content">{{ __('help.Order') }}</span>
                                                    <br>
                                                    <span class="registration-content">{{ __('help.date_of') }}</span>
                                                    <br>
                                                    <span
                                                        class="registration-content">{{ __('help.customers_details') }}</span>
                                                    <br>
                                                    <span class="registration-content">{{ __('help.Product') }}</span>
                                                    <br>
                                                    <span class="registration-content">{{ __('help.total') }}</span>
                                                    <br>
                                                    <span class="registration-content">{{ __('help.example') }}</span>
                                                    <br>
                                                <table border="1" style="background-color: white;">
                                                    <thead>
                                                        <tr>
                                                            <th>{{ __('help.OrderID') }}</th>
                                                            <th>{{ __('help.Date') }}</th>
                                                            <th>{{ __('help.Customer_Name') }}</th>
                                                            <th>{{ __('help.Products_Name') }}</th>
                                                            <th>{{ __('help.Total_Amount') }}</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr>
                                                            <td>1001</td>
                                                            <td>1.May.24</td>
                                                            <td>John Doe</td>
                                                            <td>Concrete Blocks</td>
                                                            <td>1500</td>
                                                        </tr>
                                                        <tr>
                                                            <td>1002</td>
                                                            <td>2.May.24</td>
                                                            <td>Jane Smith</td>
                                                            <td>Cement Bags</td>
                                                            <td>800</td>
                                                        </tr>
                                                    </tbody>
                                                </table>

                                                </p>
                                            </div>

                                            <span class="registration-title"><b>{{ __('help.Filter') }}</b></span>
                                            <br>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Filter_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Filter_content2') }}</span>

                                            </p>

                                            <br>

                                            <span class="registration-title"><b>{{ __('help.Export') }}</b></span>
                                            <br>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Export_content1') }}</span>
                                                <span
                                                    class="registration-content">{{ __('help.Export_content2') }}</span>

                                            </p>

                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>


                        <div class="tab-pane fade" id="Managing" role="tabpanel" aria-labelledby="Managing-tab">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-4">
                                        <div class="content">
                                            <h5 class="title-content"><b>{{ __('help.Managing') }}</b></h5>
                                            <span
                                                class="registration-title"><b>{{ __('help.Adding_New_Roles') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Adding_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Adding_content1') }}</span>

                                            </p>
                                            <br>
                                            <span
                                                class="registration-title"><b>{{ __('help.remove_New_Roles') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.remove_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.remove_content2') }}</span>

                                            </p>


                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="tab-pane fade" id="reviewingConst" role="tabpanel"
                            aria-labelledby="reviewingConst-tab">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-4">
                                        <div class="content">
                                            <h5 class="title-content"><b>{{ __('help.Reviewing-title1') }}</b></h5>
                                            <span class="registration-title"><b>{{ __('help.Reviewing') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Reviewing_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Reviewing_content2') }}</span>

                                            </p>
                                            <br>
                                            <span class="registration-title"><b>{{ __('help.Disputing') }}</b></span>
                                            <p class="registration-form-title">
                                                <span
                                                    class="registration-content">{{ __('help.Disputing_content1') }}</span>
                                                <br>
                                                <span
                                                    class="registration-content">{{ __('help.Disputing_content2') }}</span>

                                            </p>

                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>


                    </div>
                </div>
            </div>
        </div>

        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Function to update the breadcrumb
                function updateBreadcrumb(title) {
                    document.getElementById('current-page').textContent = title;
                }

                // Initial breadcrumb update with the active tab's title
                const activeTab = document.querySelector('.nav-link.active');
                if (activeTab) {
                    updateBreadcrumb(activeTab.getAttribute('data-title'));
                }

                // Add event listeners to all nav-links
                document.querySelectorAll('.nav-link').forEach(function(tab) {
                    tab.addEventListener('click', function() {
                        // Update the breadcrumb with the clicked tab's title
                        updateBreadcrumb(tab.getAttribute('data-title'));
                    });
                });
            });
        </script>
    @endsection
