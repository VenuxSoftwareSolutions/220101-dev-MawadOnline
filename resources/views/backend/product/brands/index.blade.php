                                ...
                                @foreach ($brands as $key => $brand)
                                    <tr>
                                        ...
                                        <td class="text-right">
                                            ...
                                            {{-- @can('view_brand_bu') --}}
                                            @php
                                                $buJob = $brand->buJobs->first();
                                            @endphp
                                            <a class="btn btn-soft-info btn-icon btn-circle btn-sm {{ !$buJob ? 'disabled' : '' }}"
                                                @if ($buJob) href="{{ route('brands.byBuJob', ['bu_job_id' => $buJob->id]) }}"
											@else
												href="javascript:void(0);" @endif
                                                title="{{ !$buJob ? translate('No jobs for this brand') : translate('View BU') }}">
