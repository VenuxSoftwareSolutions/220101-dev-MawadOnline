        ...
        <ul class="p-3 mb-0 list-unstyled">
            ...
            @foreach(get_level_zero_categories() as $category_level_zero)
                ...
                <li class="mb-3 ml-1 @if(in_array($category_level_zero->id,$category_parents_ids)) fw-600 @endif">
                    <a class="text-reset fs-14 hov-text-primary" href="javascript:filter_category({{$category_level_zero->id}});">
                        @if(in_array($category_level_zero->id,$category_parents_ids)) - @else + @endif{{ $category_level_zero->getTranslation('name') }}
                    </a>
                </li>
                ...
                @foreach ($category_level_zero->childrenCategories as $key => $sub_category)
                    @if(in_array($category_level_zero->id,$category_parents_ids))
                        <li class="ml-4 mb-3 @if(in_array($sub_category->id,$category_parents_ids)) fw-600 @endif @if(!in_array($category_level_zero->id,$category_parents_ids)) display_none @endif">
                            <a class="text-reset fs-14 hov-text-primary" href="javascript:filter_category({{$sub_category->id}});">
                                @if(in_array($sub_category->id,$category_parents_ids)) - @else + @endif{{ $sub_category->getTranslation('name') }}
                            </a>
                        </li>
                        ...
                        @if(in_array($sub_category->id,$category_parents_ids))
                            <li class="ml-5 mb-3 @if(in_array($immediate_children_category->id,$category_parents_ids)) fw-600 @endif @if(!in_array($sub_category->id,$category_parents_ids)) display_none @endif">
                                <a class="text-reset fs-14 hov-text-primary" href="javascript:filter_category({{$immediate_children_category->id}});">
                                    {{ $immediate_children_category->getTranslation('name') }}
                                </a>
                            </li>
                        @endif
    ...
    <div class="p-3 mr-3">
        ...
        <div class="aiz-range-slider">
            ...
            <div class="row mt-2">
                <div class="col-6">
                    <span class="range-slider-value value-low fs-14 fw-600 opacity-70"
                        @if (isset($min_price) && $min_price && $min_price>$min_max_price['min'] && $min_price<$min_max_price['max'])
                            data-range-value-low="{{ $min_price }}"
                        @else
                            data-range-value-low="{{ $min_max_price['min']}}"
                        @endif
                        id="input-slider-range-value-low"
                    ></span>
                </div>
        ...
        <div class="p-3 aiz-checkbox-list">
            <div class="form-group">
                <div class="rating rating-input">
                    <label>
                        <input type="radio" name="rating" value="1" @if(isset($rating) && $rating>0) checked @endif  onchange="filter()" required>
                        <i class="las la-star @if(isset($rating) && $rating>0) active @endif"></i>
                    </label>
                    <label>
                        <input type="radio" name="rating" value="2" @if(isset($rating) && $rating>1) checked @endif onchange="filter()">
                        <i class="las la-star @if(isset($rating) && $rating>1) active @endif"></i>
                    </label>
                    <label>
                        <input type="radio" name="rating" value="3" @if(isset($rating) && $rating>2) checked @endif onchange="filter()">
                        <i class="las la-star @if(isset($rating) && $rating>2) active @endif"></i>
                    </label>
                    <label>
                        <input type="radio" name="rating" value="4" @if(isset($rating) && $rating>3) checked @endif onchange="filter()">
                        <i class="las la-star @if(isset($rating) && $rating>3) active @endif"></i>
                    </label>
                    <label>
                        <input type="radio" name="rating" value="5" @if(isset($rating) && $rating>4) checked @endif onchange="filter()">
...
@if($attribute->type_value == "numeric")
    <div class="bg-white border mb-3">
        ...
        <div class="collapse {{ $show }}" id="collapse_{{ str_replace(' ', '_', $attribute->name) }}">
            <div class="p-3 mr-3">
                <div class="aiz-range-slider-attribute" data-id="{{$attribute->id}}">
                    <div
                        ...
                        data-range-value-max="{{$max_attribute_value}}"
                    ></div>

                    <div class="row mt-2">
                        <div class="col-6">
                            <span class="attribute-input-slider-range-value-low range-slider-value value-low fs-14 fw-600 opacity-70"
                                @if ($min_value < $min_attribute_value || $min_value > $max_attribute_value)
                                    data-range-value-low="{{ $min_attribute_value }}"
                                @else
                                    data-range-value-low="{{$min_value}}"
                                @endif
                                id="attribute-input-slider-range-value-low"
                            ></span>
                        </div>
                        <div class="col-6 text-right">
                            <span class="attribute-input-slider-range-value-high nb range-slider-value value-high fs-14 fw-600 opacity-70"
                                @if ($max_value > $max_attribute_value)
                                    data-range-value-high="{{ $max_attribute_value }}"
                                @else
                                    data-range-value-high="{{$max_value}}"
                                @endif
                                id="attribute-input-slider-range-value-high"
                            ></span>
                        </div>
                    </div>
                </div>
            ...
            <div class="min-max ">
            <input class="form-control" onchange="filter()" placeholder='{{ translate('min')}} {{$attribute->name}}' type="number" id="min_attribute_numeric_{{$attribute->id}}" name="{{$attribute->id}}[]" value="@if($min_value < $min_attribute_value || $min_value > $max_attribute_value){{ $min_attribute_value }}@else{{$min_value}}@endif">
            <input class="form-control" onchange="filter()" placeholder='{{ translate('max')}} {{$attribute->name}}' type="number" id="max_attribute_numeric_{{$attribute->id}}" name="{{$attribute->id}}[]" value="@if($max_value > $max_attribute_value){{ $max_attribute_value }}@else{{$max_value}}@endif">
            </div>
        </div>
    </div>
                ...
                @foreach ($attribute->attribute_values_filter($id_products) as $key => $attribute_value)
                    ...
                    <label class="aiz-checkbox mb-3" @if($key>4 && count($attribute->attribute_values_filter($id_products))>7) hide_attribute display_none @endif>
                        <input
                            ...
                            @if(isset($request_all[$attribute->id]) && in_array($value_attribute, $request_all[$attribute->id])) checked @endif 
                            onchange="filter()"
                        >
                        <span class="aiz-square-check"></span>
